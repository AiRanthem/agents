...
API keys may carry a static `sandbox.count` quota. Dynamic enforcement uses Redis only. If
`--e2b-quota-redis-addr` is empty, or Redis is configured but unavailable, sandbox-manager intentionally fails open:
limited keys are accepted and stored, but create requests are temporarily unenforced. Metrics and logs expose fail-open
events.
