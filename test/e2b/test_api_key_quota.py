...
def resume_sandbox(sbx):
    if hasattr(sbx, "resume"):
        return sbx.resume()
    return connect_sandbox(sbx, timeout=6000)
...
def assert_ready_sandbox_count(sandboxset_name, minimum):
    ...
    for item in data.get("items", []):
        ...
        for cond in item.get("status", {}).get("conditions", []):
            if cond.get("type") == "Ready" and cond.get("status") == "True":
                ready += 1
                break
    assert ready >= minimum, f"{sandboxset_name} ready count {ready} < {minimum}"
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "full",
            {
                "all": {"count": 5, "cpu": 500, "memory": 512},
                "running": {"count": 2, "cpu": 200, "memory": 256},
            },
            {
                "all": {"count": 5, "cpu": 500, "memory": 512},
                "running": {"count": 2, "cpu": 200, "memory": 256},
...
@pytest.mark.parametrize(
    ...
    [
        ("negative_count", {"name": "x", "quota": {"all": {"count": -1}}}, "limit"),
        ("negative_cpu", {"name": "x", "quota": {"running": {"cpu": -1}}}, "limit"),
        ("negative_memory", {"name": "x", "quota": {"all": {"memory": -1}}}, "limit"),
        ("unsupported_scope", {"name": "x", "quota": {"template:code-interpreter": {"count": 1}}}, "scope"),
...
def test_api_key_quota_zero_limit_is_valid_and_blocks_create(sandbox_context):
    ...
    owned = []
    try:
        resp = create_api_key("quota-zero", marker, quota={"all": {"count": 0}})
        assert resp.status_code == 201, resp.text
        body = resp.json()
        created_id = body["id"]
...
def test_count_all_limit_blocks_then_delete_releases(sandbox_context):
    ...
    owned = []
    try:
        resp = create_api_key("quota-count-all", marker, quota={"all": {"count": 1}})
        assert resp.status_code == 201, resp.text
        body = resp.json()
        created_id = body["id"]
        api_key = body["key"]

        first = track_sandbox(owned, create_sandbox_with_key(api_key, QUOTA_SMALL_TEMPLATE, marker))
...
def test_count_all_limit_still_counts_paused_sandbox(sandbox_context):
    ...
    owned = []
    try:
        resp = create_api_key("quota-count-all-paused", marker, quota={"all": {"count": 1}})
        assert resp.status_code == 201, resp.text
        body = resp.json()
        created_id = body["id"]
        api_key = body["key"]
...
def test_count_running_limit_pause_frees_running_scope(sandbox_context):
    ...
    owned = []
    try:
        resp = create_api_key("quota-count-running", marker, quota={"running": {"count": 1}})
        assert resp.status_code == 201, resp.text
        body = resp.json()
        created_id = body["id"]
        api_key = body["key"]
...
def test_running_resume_can_exceed_limit_and_blocks_new_creates(sandbox_context):
    ...
    owned = []
    try:
        resp = create_api_key("quota-running-resume", marker, quota={"running": {"count": 1}})
        assert resp.status_code == 201, resp.text
        body = resp.json()
        created_id = body["id"]
        api_key = body["key"]
...
def test_no_stock_path_stamps_owner_lockstring_and_counts_quota(sandbox_context):
    ...
    try:
        resp = create_api_key(
            "quota-no-stock-lock",
            marker,
            quota={"all": {"count": 1, "cpu": 2000, "memory": 2048}},
        )
        assert resp.status_code == 201, resp.text
        body = resp.json()
        created_id = body["id"]
        ...
        sbx = track_sandbox(owned, create_sandbox_with_key(api_key, QUOTA_NOSTOCK_TEMPLATE, marker))
...
def test_checkpoint_clone_path_stamps_owner_and_lockstring(sandbox_context):
    ...
    try:
        resp = create_api_key(
            ...
            marker,
            quota={"all": {"count": 2, "cpu": 1000, "memory": 1024}},
        )
        assert resp.status_code == 201, resp.text
        body = resp.json()
        created_id = body["id"]
        ...
        source = track_sandbox(owned, create_sandbox_with_key(api_key, QUOTA_SMALL_TEMPLATE, marker))
...
def test_concurrent_creates_do_not_exceed_count_limit(sandbox_context):
    ...
    owned = []
    try:
        resp = create_api_key("quota-concurrent", marker, quota={"all": {"count": 3}})
        assert resp.status_code == 201, resp.text
        body = resp.json()
        created_id = body["id"]
        api_key = body["key"]
...
def test_quota_miss_returns_e2b_error_body_and_does_not_lock_pooled_sandbox():
    ...
    owned = []
    try:
        resp = create_api_key("quota-pool-return", marker, quota={"all": {"count": 0}})
        assert resp.status_code == 201, resp.text
        body = resp.json()
        created_id = body["id"]
        api_key = body["key"]
...
def test_antidrift_releases_non_manager_deleted_sandbox(sandbox_context):
    ...
    owned = []
    try:
        resp = create_api_key("quota-antidrift-release", marker, quota={"all": {"count": 1}})
        assert resp.status_code == 201, resp.text
        body = resp.json()
        created_id = body["id"]
        api_key = body["key"]
...
def test_redis_data_loss_rebuilds_from_live_crs_and_reenforces(sandbox_context):
    ...
    owned = []
    try:
        resp = create_api_key("quota-redis-loss", marker, quota={"all": {"count": 2}})
        assert resp.status_code == 201, resp.text
        body = resp.json()
        created_id = body["id"]
        api_key = body["key"]
...
def test_redis_outage_fails_open_then_reenforces_after_rebuild(sandbox_context):
    ...
    owned = []
    try:
        resp = create_api_key("quota-redis-outage", marker, quota={"all": {"count": 1}})
        assert resp.status_code == 201, resp.text
        body = resp.json()
        created_id = body["id"]
        api_key = body["key"]
        ...
        assert first.get_info().state == SandboxState.RUNNING
        wait_until(lambda: assert_redis_count(created_id, "all", 1), timeout=120)
        wait_until(lambda: assert_ready_sandbox_count(QUOTA_SMALL_TEMPLATE, minimum=5), timeout=240)
