...
> Revised 2026-06-01. This revision supersedes the original anonymous-`/wake`-endpoint
> design. Two upstream changes drove the rewrite:
>
> 1. Commit `4ec17604` rewrote the connect/resume path: `UpdatePolicyBaselineAware`
>    and `timeout.Options.Baseline` were removed, `Resume` became atomic (it writes a
>    placeholder timeout together with `Spec.Paused=false`), and a configurable
>    minimum-timeout floor (`--e2b-min-resume-timeout`, default `300s`) is applied via
>    `getEffectivePauseTimeSeconds`. Post-resume timeout writes now use `ExtendOnly`.
> 2. Commit `c85924cf` moved several symbols (see Code Impact for the new locations).
>
> Given those changes, the wake path no longer needs a parallel manager-side
> orchestration (`WakeSandbox` / `ConnectOrWake` / a manager-side annotation parser /
> a `wakeMinimumTimeout` constant). Instead the sandbox-gateway authenticates with a
> narrowly-scoped **system credential** and calls the existing `ConnectSandbox` E2B
> endpoint, which already performs atomic resume + floored timeout + `ExtendOnly`.
...
### System credential and `AllowSystemKey` authorization

The gateway must call `ConnectSandbox` on sandboxes it does not own, without an
end-user identity. Rather than broadening the admin key into a cross-owner god
credential, a separate **system credential** is introduced with a declarative,
per-route scope whitelist. It is decoupled from the API-key storage backend.

**System key module (`pkg/servers/e2b/keys/`).** The system key is a **standalone new
module** under `pkg/servers/e2b/keys/` (e.g. `systemkey.go`), deliberately decoupled from
the existing `KeyStorage` backends (`secretKeyStorage` / `mysqlKeyStorage`) and from the
`KeyStorage` interface. It is **not** persisted in the E2B API-key store — MySQL keeps
only HMAC hashes (the plaintext could not be delivered to the gateway from there) and the
...
match it synthesizes a fixed system principal (`SystemKeyID`, cluster-scope namespace
resolution like admin, scopes `{SystemAuthConnect}`). Nothing about the system key flows
through `KeyStorage`, so the team-coalescing and MySQL-hash constraints do not apply.
`CheckApiKey` must explicitly reject blank presented keys and must never compare against
or accept a blank in-memory system key.
...
registered **before** `CheckApiKey` (middlewares execute in registration order;
`pkg/servers/web/framework.go:99`). Only the connect route opts in:
...
**`CheckApiKey` (enhanced, single auth entry).**

- Presented key is the system key:
  - Read the accepted-scopes set from context. If absent or empty (the route did not
    declare `AllowSystemKey`) → 403. This auto-restricts the system key to opted-in
    routes; on every other route a presented system key is rejected, so it can never
    create / list / delete / pause / set-timeout.
  - If the system principal's scopes intersect the route's accepted scopes → pass, skip
    the owner check, and mark `allowAnyOwner` in context. Otherwise → 403.
- Otherwise (normal user key) → existing behavior, owner check unchanged.

**Cross-owner threading.** `GetClaimedSandboxOptions` gains `AllowAnyOwner bool`. The
connect handler sets it when the context indicates a system principal, and
`GetClaimedSandbox` (`pkg/sandbox-manager/api.go:119`) skips its
`sbx.GetRoute().Owner != user` check when it is set. The system principal resolves to
cluster scope (`getNamespaceOfUser` returns `""`) like the admin team.
...
Net effect: a compromised gateway holding the system key can only trigger
connect/resume on any sandbox — it cannot delete, pause, create, or enumerate, and it
never receives sandbox access tokens. Adding a future system capability is additive:
define a new `SystemAuth` constant, grant it to the system principal, and attach
`AllowSystemKey(<new scope>)` to the relevant routes.
...
DNS / Service) and the system key (from `e2b-system-key-store`). The gateway already
...
- The gateway mounts / reads the `e2b-system-key-store` Secret to obtain the system key.
...
- `pkg/servers/e2b/routes.go` — `AllowSystemKey(auths ...SystemAuth)` middleware;
...
### System credential authorization (`pkg/servers/e2b/routes_test.go`)

- System key on connect route (`AllowSystemKey(SystemAuthConnect)`): passes,
  cross-owner, `AllowAnyOwner` set.
- System key on connect route with a gateway-retryable wake error maps to `409`.
- System key on a route without `AllowSystemKey` (delete / pause / create / list): 403.
- System key whose scopes do not include the route's accepted scope: 403.
- Normal user key on the connect route: unchanged owner check; cross-owner denied (401).
- `AllowSystemKey` runs before `CheckApiKey` (scope set visible to `CheckApiKey`).
- System key recognized in memory without a `KeyStorage` entry (both Secret and MySQL
  backends).
...
- **`e2b-system-key-store` Secret shape** — single plaintext key entry in
