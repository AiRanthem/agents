...
func CanonicalizeForLookup(apiKey string) string
func PresentCreatedAPIKey(apiKey *models.CreatedTeamAPIKey) *models.CreatedTeamAPIKey
```

`CanonicalizeForLookup` returns the decoded raw key when the presented key is a
valid OpenKruise-compatible encoded key, and the presented key unchanged
otherwise.

`PresentCreatedAPIKey` returns a deep copy of the created API key model with
...
2. Call `keys.CanonicalizeForLookup` to obtain the lookup key.
...
```go
keys.PresentCreatedAPIKey(createdAPIKey)
```
...
- `PresentCreatedAPIKey` returning a copy and preserving the original raw key.
