...
- invalid annotation value: return a parse error to the caller; each caller decides whether that error is fatal. E2B write paths and the controller fail open to the default retention and backfill `default` when their lifecycle update is accepted.
...
- If the annotation is present but `Spec.ShutdownTime` is nil, do not create a shutdown deadline. This preserves the never-timeout meaning of an absent shutdown deadline even if a direct CRD writer adds the annotation; invalid annotation values can still be backfilled to `default`.
- If the annotation value is `default`, use 100 years.
- If the annotation value is invalid, use the default retention, patch `Spec.Paused=true`, backfill the annotation to `default`, recalculate `ShutdownTime` when it is non-nil, log the problem, and emit a Warning event. The controller should not return a reconcile error after successfully patching `Paused=true`; otherwise a bad annotation would keep the sandbox running and cause repeated reconcile failures.
...
5. If annotation is invalid, controller uses default retention, backfills the annotation to `default`, records a warning, and recalculates `ShutdownTime` when it is non-nil.
...
Invalid controller annotation should not block auto-pause. The controller should patch `Spec.Paused=true`, backfill the annotation to `default`, recalculate `ShutdownTime` with the default retention when `ShutdownTime` is non-nil, log the validation error, and emit a Warning event. This deliberately favors stopping the running sandbox over enforcing retention parsing in the controller loop.
...
- Invalid annotation patches `Paused=true`, backfills `default`, recalculates `ShutdownTime` when it is non-nil, and records a warning.
