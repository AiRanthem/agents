...
E2B 对创建、列表及没有源 Sandbox 的资源操作使用 `x-e2b-kruise-infra` 选择后端；省略时选择默认后端。已有 Sandbox 的归属来自 Route，客户端通常只需提供 Sandbox ID。管理 API 先按调用方权限过滤候选，唯一可访问候选不受隐藏候选影响；可通过 Infra 消歧时，仅返回可访问的候选 Infra，由用户显式选择，同一 Infra 内仍有歧义则拒绝。Gateway 对已观测到的歧义拒绝流量访问。
...
| 无 Infra 请求头，只有一个可访问候选 | 使用该候选的 Infra，即使还存在其他用户或归属不明的记录 |
...
候选选择、访问检查和本次请求使用的资源身份及 Infra 必须保持一致，不能在后续处理时重新按默认项选择。取得选中候选对应的后端 Sandbox 后，仍检查实际资源归属和操作所需状态；Infra 选择本身不授予访问权限，后续检查失败也不转查其他候选。因选择歧义或访问检查失败而拒绝的请求，不修改任何候选资源。用户可以通过显式 Infra 查询或删除该后端内唯一可访问的冲突资源，但不能把该选择用于修改另一候选的归属。
