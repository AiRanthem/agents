...
func BuildAutoPauseOptions(now time.Time, requestedTimeout, pausedRetention time.Duration) timeout.Options {
	pauseTime := timeout.NormalizeTime(now.Add(requestedTimeout))
