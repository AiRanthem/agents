...
import (
	...
	managerconsts "github.com/openkruise/agents/pkg/sandbox-manager/consts"
)
...
const (
	...
	DefaultSingleflightPreemptionThreshold = managerconsts.DefaultSingleflightPreemptionThreshold

	// releaseLockTimeout is the timeout for the background context used during lock release.
	releaseLockTimeout = 5 * time.Minute

	// releaseLockRetryInterval is the retry interval for best-effort lock release.
	releaseLockRetryInterval = 100 * time.Millisecond
)
...
func singleflightCompete[T client.Object](
	...
	if err != nil {
		var conflict *singleflightSeqConflictError
		if errors.As(err, &conflict) {
			...
			if !ann.Done && isPreemptable(ann.LastUpdate, preemptionThreshold) {
				...
				return singleflightCompete[T](ctx, cache, fresh, key, ann, precheck, modifier, function, preemptionThreshold)
			}
...
func singleflightWait[T client.Object](
	...
	// do NOT collide with existing WaitForObjectSatisfied entries which use
	// WaitHookKey format "*type/ns/name". The watched sequence is part of the
	// key because each sequence has a different completion condition.
	waitKey := fmt.Sprintf("%s%s/%s/%d", SingleflightWaitKeyPrefix, key, cacheutils.WaitHookKey(obj), watchedSeq)
...
func releaseSingleflightLock[T client.Object](
	...
	for {
		...
		if releaseCtx.Err() != nil {
			log.Error(err, "singleflight: failed to release lock before release context expired; lock will be preempted by timeout")
			return
		}
		select {
		case <-releaseCtx.Done():
			log.Error(err, "singleflight: failed to release lock before release context expired; lock will be preempted by timeout")
			return
		case <-time.After(releaseLockRetryInterval):
		}
	}
}
