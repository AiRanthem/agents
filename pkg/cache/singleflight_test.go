...
func TestSingleflightWaitEntriesAreScopedByWatchedSeq(t *testing.T) {
	now := time.Now().Unix()
	sandbox := &v1alpha1.Sandbox{
		ObjectMeta: metav1.ObjectMeta{
			Name:      "test-sandbox",
			...
			Annotations: map[string]string{
				SingleflightAnnotationPrefix + "test-key": fmt.Sprintf("1:false:%d", now),
			},
		},
		Status: v1alpha1.SandboxStatus{
			Phase: v1alpha1.SandboxRunning,
	...
	waitCtx1, cancelWait1 := context.WithCancel(t.Context())
	defer cancelWait1()
	wait1Done := make(chan error, 1)
	go func() {
		_, waitErr := singleflightWait(waitCtx1, cache, sandbox, "test-key", 1)
		wait1Done <- waitErr
	}()

	require.Eventually(t, func() bool {
		return countWaitHooks(cache.GetWaitHooks()) == 1
	}, time.Second, 10*time.Millisecond)

	waitCtx2, cancelWait2 := context.WithCancel(t.Context())
	defer cancelWait2()
	wait2Done := make(chan error, 1)
	go func() {
		_, waitErr := singleflightWait(waitCtx2, cache, sandbox, "test-key", 2)
		wait2Done <- waitErr
	}()

	require.Eventually(t, func() bool {
		return countWaitHooks(cache.GetWaitHooks()) == 2
	}, time.Second, 10*time.Millisecond)

	updateSingleflightAnnotation(t, fc, "default", "test-sandbox", "test-key", 2, false)

	require.Eventually(t, func() bool {
		select {
		case waitErr := <-wait1Done:
			return waitErr == nil
		default:
			return false
		}
	...
	select {
	case waitErr := <-wait2Done:
		t.Fatalf("waiter for seq 2 returned before seq 2 completed: %v", waitErr)
	case <-time.After(100 * time.Millisecond):
	}

	updateSingleflightAnnotation(t, fc, "default", "test-sandbox", "test-key", 2, true)
...
func TestReleaseSingleflightLock_RetriesUntilReleaseContextSucceeds(t *testing.T) {
	now := time.Now().Unix()
	sandbox := &v1alpha1.Sandbox{
		ObjectMeta: metav1.ObjectMeta{
			Name:      "test-sandbox",
	...
	attempts := 0
	updateConflicts := 4
	cache, fc, err := newSingleflightTestCacheWithInterceptor(t, interceptor.Funcs{
		Update: func(ctx context.Context, c ctrlclient.WithWatch, obj ctrlclient.Object, opts ...ctrlclient.UpdateOption) error {
			...
			if obj.GetName() == "test-sandbox" && obj.GetNamespace() == "default" {
				attempts++
				if attempts <= updateConflicts {
					return apierrors.NewConflict(v1alpha1.Resource("sandboxes"), obj.GetName(), fmt.Errorf("forced conflict %d", attempts))
				}
			}
	...
	require.Greater(t, attempts, updateConflicts)
	var updated v1alpha1.Sandbox
	require.NoError(t, fc.Get(t.Context(), types.NamespacedName{Namespace: "default", Name: "test-sandbox"}, &updated))
	annValue := updated.Annotations[SingleflightAnnotationPrefix+"test-key"]
	assert.Contains(t, annValue, "5:true:")
...
func newSingleflightTestCache(t *testing.T, initObjs ...ctrlclient.Object) (*Cache, ctrlclient.Client, error) {
	t.Helper()
	return newSingleflightTestCacheWithInterceptor(t, resourceVersionInterceptorFuncs(), initObjs...)
}

func newSingleflightTestCacheWithInterceptor(t *testing.T, interceptorFuncs interceptor.Funcs, initObjs ...ctrlclient.Object) (*Cache, ctrlclient.Client, error) {
	t.Helper()
...
func countWaitHooks(waitHooks *sync.Map) int {
	count := 0
	waitHooks.Range(func(_, _ any) bool {
		count++
		return true
	})
	return count
...
func updateSingleflightAnnotation(t *testing.T, c ctrlclient.Client, namespace, name, key string, seq int64, done bool) {
	t.Helper()
	var sbx v1alpha1.Sandbox
	require.NoError(t, c.Get(t.Context(), types.NamespacedName{Namespace: namespace, Name: name}, &sbx))
	modified := sbx.DeepCopy()
	setSingleflightAnnotation(modified, key, seq, done)
	require.NoError(t, c.Update(t.Context(), modified))
}
