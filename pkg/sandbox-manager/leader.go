...
func newPrimaryElector(opts config.SandboxManagerOptions, state *primaryState) (*primaryElector, error) {
	...
	client, err := newPrimaryElectionClient(opts.RestConfig)
	if err != nil {
		return nil, err
	}
	lock, err := resourcelock.New(
		...
		resourcelock.ResourceLockConfig{Identity: primaryIdentity()},
	)
	if err != nil {
		return nil, err
	}
...
func newPrimaryElectionClient(cfg *rest.Config) (kubernetes.Interface, error) {
	leaseConfig := *cfg
	timeout := primaryRenewDeadline / 2
	if timeout < time.Second {
		timeout = time.Second
	}
	leaseConfig.Timeout = timeout
	return kubernetes.NewForConfig(rest.AddUserAgent(&leaseConfig, "sandbox-manager-primary-election"))
}

func primaryIdentity() string {
