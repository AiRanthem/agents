...
func (s *Sandbox) SingleflightDo(
	...
	return cache.DistributedSingleFlightDo(
		...
		modifier,
		function,
		s.getSingleflightPreemptionThreshold(),
	)
}

func (s *Sandbox) getSingleflightPreemptionThreshold() time.Duration {
	if s.singleflightPreemptionThreshold <= 0 {
		return cache.DefaultSingleflightPreemptionThreshold
	}
	return s.singleflightPreemptionThreshold
}
...
func (s *Sandbox) Resume(ctx context.Context, opts infra.ResumeOptions) (retErr error) {
	...
	resumeModifier := func(sbx *agentsv1alpha1.Sandbox) {
		// Idempotent short-circuit: if another resumer already flipped Paused
		// to false, do not re-apply bumpResumeTimeoutProtection. Without this,
		// a late resumer entering Compete after the previous runner released
		// (ann.Done=true) but before Status.Phase has been observed as Running
		// would bump T_pause/T_shutdown back to now+1h, silently overwriting
		// the SaveTimeout the previous resumer just persisted.
		if !sbx.Spec.Paused {
			return
		}
