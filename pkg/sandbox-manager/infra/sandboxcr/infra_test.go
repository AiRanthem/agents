...
func TestInfra_GetClaimedSandboxWithOptions_NamespaceScoped(t *testing.T) {
	...
	require.ErrorIs(t, err, infracache.ErrSandboxNotFound)
	assert.Nil(t, got)
}
...
func TestInfra_GetSandbox_RetainsRouteRVObservedBeforePoll(t *testing.T) {
	...
	setRouteResourceVersion(infraInstance, id, "20")
	retryCache.afterCall = func(call int64) {
		if call == 2 {
			delete(infraInstance.Routes.(stubRouteReader), id)
		}
	}
...
func TestInfra_GetSandbox_StalenessFallback(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			apiSbx.ResourceVersion = "20"
			if tt.sandboxID != "" {
				apiSbx.Labels[v1alpha1.LabelSandboxID] = tt.sandboxID
			}
			cacheSbx := apiSbx.DeepCopy()
			...
			if tt.routeRV != "" {
				setRouteResourceVersion(infraInstance, id, tt.routeRV)
			}
