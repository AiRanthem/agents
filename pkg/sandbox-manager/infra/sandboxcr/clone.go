...
func CloneSandbox(ctx context.Context, opts infra.CloneSandboxOptions, cache infracache.Provider) (cloned infra.Sandbox, metrics infra.CloneMetrics, err error) {
	...
	sbx, metrics, createAttempted, err = createPreparedSandbox(ctx, opts, sbx, cache, metrics)
	if err != nil {
		if admitted && isKnownRejectedSandboxWrite(err) {
			releaseAdmission(ctx, opts.Admission, opts.LockString)
			admitted = false
		}
...
func createPreparedSandbox(ctx context.Context, opts infra.CloneSandboxOptions, sbx *Sandbox, cache infracache.Provider, metrics infra.CloneMetrics) (*Sandbox, infra.CloneMetrics, bool, error) {
	...
	if err != nil {
		...
		return nil, metrics, createAttempted, err
	}
	sbx.Sandbox = created
	...
	return sbx, metrics, createAttempted, nil
}
