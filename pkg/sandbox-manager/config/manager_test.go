...
func TestValidateQuotaRedisTimeout(t *testing.T) {
	tests := []struct {
		...
		{name: "positive timeout allowed", timeout: 50 * time.Millisecond},
		{name: "zero timeout rejected", timeout: 0, expectError: "quota redis operation timeout must be greater than 0"},
		{name: "negative timeout rejected", timeout: -time.Millisecond, expectError: "quota redis operation timeout must be greater than 0"},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
