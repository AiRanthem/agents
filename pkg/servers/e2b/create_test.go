...
func TestMapCloneSandboxError(t *testing.T) {
	tests := []struct {
		name       string
		err        error
		expectCode int
		...
		{
			name:       "manager conflict maps to 409",
			err:        managererrors.NewError(managererrors.ErrorConflict, "sandbox already exists"),
			expectCode: http.StatusConflict,
		},
		{
			name:       "generic error has no code",
			err:        stderrors.New("some internal failure"),
			expectCode: 0,
		},
		{
			name:       "raw apierrors AlreadyExists is not specially mapped (manager is expected to wrap)",
			err:        apierrors.NewAlreadyExists(schema.GroupResource{Group: "agents.kruise.io", Resource: "sandboxes"}, "my-sbx"),
			expectCode: 0,
		},
		{
			name:       "NotFound is not specially mapped",
			err:        apierrors.NewNotFound(schema.GroupResource{Group: "agents.kruise.io", Resource: "sandboxes"}, "x"),
			expectCode: 0,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			apiErr := mapCloneSandboxError(tt.err)
			require.NotNil(t, apiErr)
			assert.Equal(t, tt.expectCode, apiErr.Code)
			assert.Equal(t, tt.err.Error(), apiErr.Message)
		})
	}
}
