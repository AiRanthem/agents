...
func (sc *Controller) ResumeSandbox(r *http.Request) (web.ApiResponse[struct{}], *web.ApiError) {
	...
	if err := sc.manager.ResumeSandbox(ctx, sbx, infra.ResumeOptions{Timeout: &timeoutOptions}); err != nil {
		if errors.Is(err, infra.ErrResumeCompletedByOtherRequest) {
			if apiErr := sc.updateTimeoutAfterFollowerResume(ctx, sbx, request.TimeoutSeconds); apiErr != nil {
				return web.ApiResponse[struct{}]{}, apiErr
			}
			return web.ApiResponse[struct{}]{
				Code: http.StatusNoContent,
	...
	if state == v1alpha1.SandboxStatePaused {
		...
		if err := sc.manager.ResumeSandbox(ctx, sbx, infra.ResumeOptions{Timeout: &timeoutOptions}); err != nil {
			if errors.Is(err, infra.ErrResumeCompletedByOtherRequest) {
				...
				log.Info("sandbox resume completed by another request", "timeout", sbx.GetTimeout())
			} else {
...
func (sc *Controller) updateTimeoutAfterFollowerResume(ctx context.Context, sbx infra.Sandbox, timeoutSeconds int) *web.ApiError {
	if err := sbx.InplaceRefresh(ctx, false); err != nil {
		return &web.ApiError{
			Message: fmt.Sprintf("Failed to refresh sandbox after resume: %v", err),
		}
	}
	autoPause, currentEndAt := ParseTimeout(sbx)
	return sc.updateConnectTimeout(ctx, sbx, timeoutSeconds, v1alpha1.SandboxStateRunning, autoPause, currentEndAt)
}
