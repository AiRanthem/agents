...
func TestQuotaSubjectListerListLimited(t *testing.T) {
	...
	store := &fakeKeyStorageForSubjects{
		keys: []*models.CreatedTeamAPIKey{
			{ID: limitedID, QuotaSpec: &models.QuotaSpec{Limits: []models.QuotaLimit{{
				Dimension: models.DimSandboxCount, Scope: models.ScopeRunning, Limit: 5,
			}}}},
			{ID: unlimitedID, QuotaSpec: &models.QuotaSpec{}},
		...
		byID: map[string]*models.CreatedTeamAPIKey{
			limitedID.String(): {
				...
				QuotaSpec: &models.QuotaSpec{Limits: []models.QuotaLimit{{
					Dimension: models.DimLimitsCPU, Scope: models.ScopeRunning, Limit: 4000,
				}}},
			},
			"unlimited": {
				ID:        uuid.New(),
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.wantOK {
				...
				assert.False(t, ok)
				assert.Equal(t, quota.Subject{}, subject)
			}
		})
	}
}
