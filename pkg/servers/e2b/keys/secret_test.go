...
func TestSecretKeyStorage_CreateKey_TriggersRefresh(t *testing.T) {
	storage, _ := newSecretStorageForTest(t, map[string][]byte{})

	// Drain any signal that may have been queued during prior setup.
	select {
	case <-storage.refreshC:
	default:
	}

	owner := &models.CreatedTeamAPIKey{ID: AdminKeyID, Team: models.AdminTeam()}
	_, err := storage.CreateKey(t.Context(), owner, CreateKeyOptions{Name: "k", TeamName: models.AdminTeamName})
	require.NoError(t, err)

	select {
	case <-storage.refreshC:
	default:
		t.Fatal("expected refresh signal after CreateKey")
	}
}

func TestSecretKeyStorage_DeleteKey_TriggersRefresh(t *testing.T) {
	storage, _ := newSecretStorageForTest(t, map[string][]byte{})

	owner := &models.CreatedTeamAPIKey{ID: AdminKeyID, Team: models.AdminTeam()}
	created, err := storage.CreateKey(t.Context(), owner, CreateKeyOptions{Name: "k", TeamName: models.AdminTeamName})
	require.NoError(t, err)

	// Drain the signal from CreateKey.
	select {
	case <-storage.refreshC:
	default:
	}

	require.NoError(t, storage.DeleteKey(t.Context(), created))

	select {
	case <-storage.refreshC:
	default:
		t.Fatal("expected refresh signal after DeleteKey")
	}
}
