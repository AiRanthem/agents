...
func (sc *Controller) initSystemKey() {
	...
	sc.systemKey = keys.NewSystemKey(sc.cache.GetClient(), sc.cache.GetAPIReader(), sc.systemNamespace)
}
...
func (sc *Controller) Run() (context.Context, error) {
	...
	if sc.systemKey != nil {
		klog.InfoS("ensuring system-key Secret is populated", "namespace", sc.systemNamespace, "secret", keys.SystemKeySecretName)
		if err := sc.systemKey.EnsureKey(ctx); err != nil {
			cancel()
			return nil, fmt.Errorf("ensure system key: %w", err)
		}
	}
