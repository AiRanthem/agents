...
`DeletionTimestamp`, it authoritatively deletes by ObjectKey. When an equal-RV event meets a
deletion fence, the gateway adapter enqueues the affected ObjectKey for asynchronous direct-reader
repair and completes the current reconcile without waiting for an API call.
...
`<namespace>--<name>` form; the receiver splits at the first separator, fills the ObjectKey, and
then applies ordinary full-Route validation. An opaque/short ID-only payload, a partial ObjectKey,
or a payload missing ID, UID, or resourceVersion receives `400 Bad Request` without Store mutation.
...
| Delete source | Rule |
|---|---|
| Local NotFound/DeletionTimestamp or successful manager delete/recycle trigger | Authoritatively delete the ObjectKey's current full record |
...
| Legacy ID-only peer delete | Normalize the reversible legacy ID to ObjectKey at the HTTP boundary, then apply the full peer-delete rule |
...
4. New receivers normalize reversible legacy ID-only messages to full Routes at the peer boundary.
...
| Deletes and compatibility | Old UID/RV deletes, legacy peer boundary normalization, old/new Route JSON compatibility, HTTP result mapping |
